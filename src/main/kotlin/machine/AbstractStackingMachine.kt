...
abstract class AbstractStackingMachine(
    group: ItemGroup,
    ...
    protected enum class MachineState(
    ...
    protected open val CACHE = buildMenu(
        "&f堆叠机器",
        "########!",
        "#   #IiI#",
        "#   m#n##",
            ...
            '!' -> buildItemStack(
                ...
                override fun onClick(
                    ...
                    MenuManager.openMenu(
                        if (machineInfo == null) MachineMenu(data, RecipeType.MACHINE)
                        else RecipesMenu(machineInfo, RecipeType.MACHINE, MachineMenu(data, RecipeType.MACHINE)),
                        p
                    )
                    return true
                }
    ...
    protected open fun updateMachineState(config: Config, selfBlockMenu: BlockMenu, state: MachineState, vararg lore: String) {
        config.state = state
        val item = state.getDisplay(*lore)
        CACHE.updateSlots(selfBlockMenu.toInventory(), 'm', item)
    }

    protected open fun updateMachineState(config: Config, selfBlockMenu: BlockMenu, state: MachineState, lore: List<Component>) {
        config.state = state
        val item = state.getDisplay(lore)
        CACHE.updateSlots(selfBlockMenu.toInventory(), 'm', item)
    }

    protected open fun updateStorageState(selfBlockMenu: BlockMenu, id: String?, count: Int) {
        val item = id?.let {
    ...
    protected val templateSlots by lazy { CACHE.getSlots(' ') }
    protected val inputSlot by lazy { CACHE.getSlot('i') }
    protected val outputSlot by lazy { CACHE.getSlot('o') }
    protected open fun tick(b: Block, config: Config) {
        val selfBlockMenu = BlockStorage.getInventory(b)

        // 检查网络连接
        val (rootLocation, root) = searchNetwork(b.location) ?: run {
            updateMachineState(config, selfBlockMenu, MachineState.DISCONNECT_TO_NETWORK)
            return
        }
        ...
        val energyNet = Slimefun.getNetworkManager()
            ...
            .orElse(null)
        if (energyNet == null) {
            updateMachineState(config, selfBlockMenu, MachineState.DISCONNECT_TO_ENERGY)
        ...
        if (progress != null) {
            ...
            updateMachineState(config, selfBlockMenu, MachineState.RUN, lore)
            PL.debug(b.location) {
                val percentage = String.format("%.2f", 100.0 * progress.progress / progress.recipe.duration)
        ...
        val (internalMachineId, count) = run {
            ...
            if (output == null || output.type == Material.AIR) {
                ...
                if (internalId == null || config.count == 0) {
                    updateMachineState(config, selfBlockMenu, MachineState.EMPTY_MACHINE)
                    updateStorageState(selfBlockMenu, null, 0)
        ...
        updateStorageState(selfBlockMenu, internalMachineId, count)
        ...
        if (machineTemplate == null) {
            updateMachineState(config, selfBlockMenu, MachineState.UNSUPPORTED_MACHINE)
        ...
        if (inputTemplate.isEmpty()) {
            ...
            val r = machineTemplate.empty.firstOrNull { recipe ->
                ...
                updateMachineState(config, selfBlockMenu, MachineState.LAKE_TEMPLATE)
        ...
        if (recipe == null) {
            updateMachineState(config, selfBlockMenu, MachineState.UNKNOWN_RECIPE)
        ...
        val calculatedInput = recipe.input.map { recipeItem ->
            ...
            if (entry == null) {
                updateMachineState(config, selfBlockMenu, MachineState.LAKE_MATERIAL)
    ...
    protected open fun onPlayerPlace(e: BlockPlaceEvent) {
    ...
    protected open fun onPlayerBreak(e: BlockBreakEvent, drops: MutableList<ItemStack>) {
    ...
    init {
        addItemHandler(
            object : BlockTicker() {
                ...
                override fun tick(b: Block, selfSfItem: SlimefunItem, config: Config) = this@AbstractStackingMachine.tick(b, config)
                override fun isSynchronized() = true
            },
            object : BlockPlaceHandler(false) {
                override fun onPlayerPlace(e: BlockPlaceEvent) = this@AbstractStackingMachine.onPlayerPlace(e)
            },
            object : BlockBreakHandler(false, false) {
                override fun onPlayerBreak(e: BlockBreakEvent, item: ItemStack, drops: MutableList<ItemStack>) = this@AbstractStackingMachine.onPlayerBreak(e, drops)
            }
        )
    }
     ...
     * 搜索相邻的网络, 并返回其root位置
     */
    protected fun searchNetwork(location: Location): Pair<Location, NetworkRoot>? {
